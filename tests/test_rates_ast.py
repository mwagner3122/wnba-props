"""Phase 6: NegativeBinomial assists helpers and as-of leakage checks."""
...
from src.rates_ast_data import add_asof_player_history, variance_to_mean_report
from src.rates_ast_eval import crps_from_samples, pit_from_samples
from src.rates_ast_model import RatesAstModel, _fit_nb_r


class TestRatesAst(unittest.TestCase):
    def test_asof_history_no_leakage(self):
        df = pd.DataFrame(
            {
                ...
                "ast": [3, 6, 1],
                "played": [1, 1, 1],
                "ast_p40_l10": [0.0, 0.0, 0.0],
                "ast_p40_std": [0.0, 0.0, 0.0],
                "games_played": [0, 1, 2],
        ...
        self.assertEqual(out.iloc[0]["prior_ast"], 0.0)
        self.assertEqual(out.iloc[1]["prior_ast"], 3.0)
        self.assertEqual(out.iloc[1]["prior_minutes"], 20.0)
        self.assertEqual(out.iloc[2]["prior_ast"], 9.0)
        self.assertEqual(out.iloc[2]["std_ast"], 9.0)
    ...
    def test_player_posterior_shrinks_small_sample(self):
        ...
        self.assertGreater(cold["ast_shrink_weight"], 0.95)
        hot = model.player_posterior("G", 400, 2000)
        self.assertLess(hot["ast_shrink_weight"], cold["ast_shrink_weight"])
    ...
    def test_variance_report_keys(self):
        ...
        self.assertIn("var_to_mean", rep["ast"])
        self.assertGreater(rep["ast"]["var_to_mean"], 0.0)

    def test_sample_ast_not_poisson_narrow(self):
        cfg = {
            "ast_k_league": 80.0,
            "ast_k_role": 20.0,
            "opp_k": 25.0,
