"""Phase 6: NegativeBinomial points helpers and as-of leakage checks."""
...
from src.rates_pts_data import add_asof_player_history, variance_to_mean_report
from src.rates_pts_eval import crps_from_samples, pit_from_samples
from src.rates_pts_model import RatesPtsModel, _fit_nb_r


class TestRatesPts(unittest.TestCase):
    def test_asof_history_no_leakage(self):
        df = pd.DataFrame(
            {
                ...
                "pts": [10, 18, 4],
                "played": [1, 1, 1],
                "pts_p40_l10": [0.0, 0.0, 0.0],
                "pts_p40_std": [0.0, 0.0, 0.0],
                "games_played": [0, 1, 2],
        ...
        self.assertEqual(out.iloc[0]["prior_pts"], 0.0)
        self.assertEqual(out.iloc[1]["prior_pts"], 10.0)
        self.assertEqual(out.iloc[1]["prior_minutes"], 20.0)
        self.assertEqual(out.iloc[2]["prior_pts"], 28.0)
        self.assertEqual(out.iloc[2]["std_pts"], 28.0)
    ...
    def test_player_posterior_shrinks_small_sample(self):
        ...
        model.fit(train)
        cold = model.player_posterior("G", 0, 0)
        self.assertGreater(cold["pts_shrink_weight"], 0.95)
        hot = model.player_posterior("G", 800, 2000)
        self.assertLess(hot["pts_shrink_weight"], cold["pts_shrink_weight"])
    ...
    def test_variance_report_keys(self):
        ...
        self.assertIn("var_to_mean", rep["pts"])
        self.assertGreater(rep["pts"]["var_to_mean"], 0.0)

    def test_sample_pts_not_poisson_narrow(self):
        cfg = {
            "pts_k_league": 80.0,
            "pts_k_role": 20.0,
            "opp_k": 25.0,
