...
def main(argv: list[str] | None = None) -> int:
    ...
    sub.add_parser(
        "check-staleness",
        help="Fail if newest odds snapshot is older than automation.stale_hours (phase 10)",
    )
    sub.add_parser(
        "weekly-summary",
        help="Write reports/weekly_automation_summary.md from run log (phase 10)",
    )
    sub.add_parser(
        "backup",
        help="Weekly DB + raw odds JSON dump into backups/<stamp>/ (phase 10)",
    )
    ...
    if args.command == "weekly-summary":
        from src.automation import weekly_summary

        return weekly_summary()
    if args.command == "backup":
        from src.automation import weekly_backup
