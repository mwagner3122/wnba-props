"""CRPS / PIT / baseline evaluation for the rebounds NB rate model."""
...
from src.rates_reb_model import RatesRebModel
...
def interpret_pit(pits, n_bins=10):
    ...
    if max_dev < 0.25 and abs(slope) < 0.35:
        ...
        words = (
            ...
            f"{direction} rebounds on average"
        )
    else:
        shape, words = (
            "mildly irregular",
...
def save_pit_histogram(pits, path, *, title, interpretation):
    ...
    ax.hist(pits, bins=10, range=(0, 1), density=True, color="#55A868", edgecolor="white")
...
def _baseline_samples_rate_x_minutes(
    minutes, hist_reb, hist_minutes, *, n_sim, rng, league_reb_p40, nb_r
):
    n = len(minutes)
    reb_p40 = np.where(
        hist_minutes > 0,
        (hist_reb / np.maximum(hist_minutes, 1e-9)) * 40.0,
        league_reb_p40,
    )
    mu = np.clip(reb_p40 * (minutes / 40.0), 0.0, None)
...
def evaluate_reb(model: RatesRebModel, test: pd.DataFrame, cfg: dict[str, Any]) -> dict[str, Any]:
    min_m = float(cfg["min_minutes_eval"])
    eval_df = test[test["minutes"] >= min_m].copy()
    print("", flush=True)
    print("=" * 72, flush=True)
    print(
        "REB BACKTEST CONDITIONS ON REALIZED MINUTES (rate model only). "
    ...
    n_sim = int(cfg["n_sim"])
    y = params["reb"].to_numpy(dtype=float)
    minutes = params["minutes"].to_numpy(dtype=float)
    model_samples = model.sample_reb(
        minutes,
        params["reb_p40"].to_numpy(dtype=float),
        params["opp_reb_factor"].to_numpy(dtype=float),
        n_sim=n_sim,
        rng=rng,
    )
    std_samples = _baseline_samples_rate_x_minutes(
        minutes,
        params["std_reb"].to_numpy(dtype=float),
        params["std_minutes"].to_numpy(dtype=float),
        n_sim=n_sim,
        rng=rng,
        league_reb_p40=model.league_reb_p40,
        nb_r=model.nb_r,
    )
    l10_samples = _baseline_samples_rate_x_minutes(
        minutes,
        params["l10_reb"].to_numpy(dtype=float),
    ...
    save_pit_histogram(
        pits,
        pit_path,
        title=f"Rebounds PIT histogram (n={len(pits)})",
    ...
    print("", flush=True)
    print("=== Rebounds holdout comparison (CRPS lower is better) ===", flush=True)
    ...
    print("", flush=True)
    print("=== Opponent reb-allowed factors (TEAM level only) ===", flush=True)
    print(
    ...
    opp_rows = sorted(
        (
            (oid, model.opp_reb_factor[oid], model.opp_sample_size[oid])
            for oid in model.opp_reb_factor
        ),
        key=lambda t: -t[2],
    )
    ...
    return {
        ...
        "league_reb_p40": float(model.league_reb_p40),
        "role_reb_p40": dict(model.role_reb_p40),
        "reb_k_role": float(model.reb_k_role),
        "opp_k": float(model.opp_k),
        "opp_n_teams": len(model.opp_reb_factor),
