"""Load as-of frames and variance diagnostics for the rebounds rate model."""
...
def rates_reb_config(cfg: dict[str, Any]) -> dict[str, Any]:
    """Tunables for rebounds hierarchical EB NB (all magic numbers from config)."""
    r = dict(cfg.get("rates_reb") or {})
    ...
    r.setdefault("reb_k_league", 80.0)
    r.setdefault("reb_k_role", 20.0)
    ...
    r.setdefault("metrics_path", "reports/rates_reb_metrics.json")
    r.setdefault("pit_histogram_path", "reports/rates_reb_pit.png")
    r.setdefault("shrinkage_summary_path", "reports/rates_reb_shrinkage.csv")
    r.setdefault("min_minutes_eval", 1.0)
    return r


def load_rates_reb_frame(conn: sqlite3.Connection) -> pd.DataFrame:
    """Join features + box score reb/minutes for rate modeling."""
    df = pd.read_sql_query(
        """
        SELECT
            f.game_id, f.player_id, f.game_date, f.season, f.team_id, f.opponent_id,
            f.position, f.reb_p40_l10, f.reb_p40_std, f.games_played,
            f.player_form_prior_weight, f.opp_pace_shrunk, f.team_pace_shrunk,
            pg.minutes AS minutes, pg.reb AS reb, pg.oreb AS oreb, pg.dreb AS dreb,
            g.overtime_periods
    ...
    for c in ("minutes", "reb", "oreb", "dreb"):
...
def add_asof_player_history(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["prior_reb"] = g["reb"].cumsum().shift(1).fillna(0.0)
    ...
    out["std_reb"] = gs["reb"].cumsum().shift(1).fillna(0.0)
    out["std_minutes"] = gs["minutes"].cumsum().shift(1).fillna(0.0)
    out["l10_reb"] = g["reb"].transform(lambda s: s.shift(1).rolling(10, min_periods=1).sum())
...
def variance_to_mean_report(df: pd.DataFrame) -> dict[str, Any]:
    ...
    _vm(played["reb"], "reb")
    _vm(played["oreb"], "oreb")
    _vm(played["dreb"], "dreb")
    played = played.copy()
    played["reb_p40"] = played["reb"] / played["minutes"].clip(lower=1e-6) * 40.0
    _vm(played["reb_p40"], "reb_p40")
    return report
