"""CRPS / PIT / baseline evaluation for the points NB rate model."""
...
from src.rates_pts_model import RatesPtsModel
...
def _baseline_samples_rate_x_minutes(
    minutes, hist_pts, hist_minutes, *, n_sim, rng, league_pts_p40, nb_r
):
    n = len(minutes)
    pts_p40 = np.where(
        hist_minutes > 0,
        (hist_pts / np.maximum(hist_minutes, 1e-9)) * 40.0,
        league_pts_p40,
    )
    mu = np.clip(pts_p40 * (minutes / 40.0), 0.0, None)
...
def evaluate_pts(model: RatesPtsModel, test: pd.DataFrame, cfg: dict[str, Any]) -> dict[str, Any]:
    min_m = float(cfg["min_minutes_eval"])
    eval_df = test[test["minutes"] >= min_m].copy()
    print("", flush=True)
    print("=" * 72, flush=True)
    print(
        "PTS BACKTEST CONDITIONS ON REALIZED MINUTES (rate model only). "
    ...
    n_sim = int(cfg["n_sim"])
    y = params["pts"].to_numpy(dtype=float)
    minutes = params["minutes"].to_numpy(dtype=float)
    model_samples = model.sample_pts(
        minutes,
        params["pts_p40"].to_numpy(dtype=float),
        params["opp_pts_factor"].to_numpy(dtype=float),
        n_sim=n_sim,
        rng=rng,
    )
    std_samples = _baseline_samples_rate_x_minutes(
        minutes,
        params["std_pts"].to_numpy(dtype=float),
        params["std_minutes"].to_numpy(dtype=float),
        n_sim=n_sim,
        rng=rng,
        league_pts_p40=model.league_pts_p40,
        nb_r=model.nb_r,
    )
    l10_samples = _baseline_samples_rate_x_minutes(
        minutes,
        params["l10_pts"].to_numpy(dtype=float),
    ...
    save_pit_histogram(
        pits,
        pit_path,
        title=f"Points PIT histogram (n={len(pits)})",
    ...
    print("", flush=True)
    print("=== Points holdout comparison (CRPS lower is better) ===", flush=True)
    ...
    print("", flush=True)
    print("=== Opponent pts-allowed factors (TEAM level only) ===", flush=True)
    print(
    ...
    opp_rows = sorted(
        (
            (oid, model.opp_pts_factor[oid], model.opp_sample_size[oid])
            for oid in model.opp_pts_factor
        ),
        key=lambda t: -t[2],
    )
    ...
    return {
        ...
        "league_pts_p40": float(model.league_pts_p40),
        "role_pts_p40": dict(model.role_pts_p40),
        "pts_k_role": float(model.pts_k_role),
        "opp_k": float(model.opp_k),
        "opp_n_teams": len(model.opp_pts_factor),
