...
def _log_odds_run(
    *,
    ...
    """Append phase-10 run-log line (never raises)."""
    try:
        from src.automation import write_run_log_from_db

        rem = None
        if quota:
            rem = quota.get("x-requests-remaining")
        write_run_log_from_db(
            command="update-odds" + ("-dry-run" if dry_run else ""),
            records_fetched=inserted,
            errors=errors,
            remaining_quota="N/A" if dry_run or rem is None else rem,
            exit_code=exit_code,
            config_path=config_path,
        )
    except Exception as exc:  # noqa: BLE001 -- logging must not break ingest
        logger.warning("Could not append run log: %s", exc)
